...
inline float polyBlep2(float t,float dt,float idt){
    if (t < dt) {
      t *= idt;
      return bleptable[ (uint16_t)(t*256) ]-1;
    } else if (t > 8192.0 - dt) {
      t = -(t - 8192.0) * idt;
      return 1-bleptable[ (uint16_t)(t*256) ];
    }
...
//  float ampDiff = envelope(osc);
if (osc->released) {osc->amplitude=0;osc->alive=0;}
else osc->amplitude=osc->velocity;
